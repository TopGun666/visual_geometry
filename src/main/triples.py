...
class KeyFrameTriple(object):
    ...
    def __stereo_reconstruction(self):
        ...
        # compute projection matrixes for camera #1 and #2
        M_l = np.hstack((self.f1.R, self.f1.t)) #np.hstack((self.f1.R, self.f1.t))
        M_r = np.hstack((self.f2.R, self.f2.t))
        P_l = np.dot(camera.K,  M_l)
        P_r = np.dot(camera.K,  M_r)
    ...
    def recover_cameras(self):
        if len(self.f1.R) and len(self.f1.t) and len(self.f2.R) and len(self.f2.t):
            # case when we need to recover 3rd camera via first two
            return self.__stereo_reconstruction()

        else: # not self.f1.R and not self.f1.t and not self.f2.R and not self.f2.t:
